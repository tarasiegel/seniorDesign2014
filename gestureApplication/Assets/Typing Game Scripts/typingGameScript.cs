...
public class typingGameScript : typingGameBase {
	...
	class GestureTracker {
		...
		private void addToGestureSequence(GestureObject g){
			...
			if (letter == '<') {
				...
				bool isCorrect = prompt.sentence.checkLetter(letter.ToString());
				if (isCorrect) {
					prompt.proceedToNextLetter();
				}
	...
	void updateStateOfGame(){
		UI.FinishedText = gestureTracker.Prompt.sentence.getLettersCompleted();
		UI.CurrentLetter = gestureTracker.Prompt.sentence.getcurrentLetter ();
		UI.TextLeft = gestureTracker.Prompt.sentence.getLettersToGo ();
	}
